...
func _on_body_entered(_hit_body):
	if _hit_body.is_in_group(target_group):
		if _hit_body.has_method("hit"):
			hit_target.emit()
			var hit_info = current_equipment.equipment_info.duplicate()
			var attacker_stats = player_node.get_node_or_null("PlayerStats")
			if attacker_stats:
				# Resource power is an integer in the original template. Use a float
				# payload so every ATK rank improves damage without mutating gear.
				_hit_body.hit(player_node, {"power": float(hit_info.power) * attacker_stats.attack_multiplier()})
			else:
				_hit_body.hit(player_node, hit_info)
