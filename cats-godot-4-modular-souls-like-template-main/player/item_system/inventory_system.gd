...
func _ready():
	...
	var first_path: String = starter_item.resource_path
	var second_path: String = starter_item2.resource_path
	starter_item = starter_item.duplicate()
	starter_item2 = starter_item2.duplicate()
	starter_item.set_meta("source_path", first_path)
	starter_item2.set_meta("source_path", second_path)
...
func change_item(_start_index,_destination_index):
	...
	if signaling_node:
		signaling_node.current_item = current_item
		signaling_node.item_change_ended.emit(current_item)
