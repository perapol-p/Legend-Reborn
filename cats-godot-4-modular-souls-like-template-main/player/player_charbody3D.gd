...
signal damage_taken(by_what) # to indicate the damage value
...
signal item_change_ended(current_item)
...
func _ready():
	stats = preload("res://player/stats/player_stats.gd").new()
	stats.name = "PlayerStats"
	add_child(stats)
	var stats_menu = preload("res://ui/stats/stats_menu.gd").new()
	stats_menu.name = "CharacterStatsMenu"
	add_child(stats_menu)
	if has_node("GUI/HealthBar"):
		get_node("GUI/HealthBar").hide()
	if animation_tree:
	...
	get_node("/root/GameSession").call_deferred("register_player", self)
...
func _unhandled_input(_event:InputEvent):
	...
	# Escape is handled by the persistent Pause menu.
...
func attack():
	if busy or dodging or not stats.spend_stamina(25.0 if secondary_action else 15.0):
		return
	trigger_event("attack_started")
	
func attack_strong():
	if busy or dodging or not stats.spend_stamina(25.0):
		return
	secondary_action = true
	trigger_event("attack_started")
...
func dodge_or_sprint():
	if sprint_timer.is_stopped():
		...
		if !dodging && input_dir && stats.stamina >= 5.0:
				sprinting = true
...
func dodge(): 
	if dodging or busy or not stats.spend_stamina(25.0):
		return
...
func use_gadget(): # Gadget attacks consume stamina too.
	if busy or dodging or not stats.spend_stamina(20.0):
		return
...
func hit(_who, _by_what):
	...
	if parry_active:
		...
		if _who.has_method("parried"):
			_who.parried()
		return
	elif guarding and stats.spend_stamina(12.0):
		block()
	else:
		end_guard()
...
func jump():
	# Handle jump.
	if is_on_floor() and not busy and stats.spend_stamina(10.0):
		jump_started.emit()
...
func set_root_move(delta):
	...
	if current_state == state.FREE and not busy and not dodging:
		new_velocity.x *= stats.movement_multiplier()
		new_velocity.z *= stats.movement_multiplier()
