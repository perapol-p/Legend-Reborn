...
func run_tests() -> void:
	...
	await wait_world()
	check(current_scene.scene_file_path == session.HOME, "New Game starts at Home")
	session.enter_dungeon()
	await wait_world()
	check(current_scene.scene_file_path == session.WORLD, "Home gate loads the dungeon")
	...
	await wait_world()
	check(current_scene.scene_file_path == session.HOME, "Continue starts at Home")
	session.enter_dungeon()
	await wait_world()
	...
	check(current_scene.get_node_or_null(defeated_path) != null, "portal after Continue starts fresh enemies")
	check(not current_scene.get_node(object_path).opened, "new expedition resets its chests")
	check(current_scene.generation_seed != snapshot.dungeon_seed, "portal after Continue generates a new layout")
	pause.resume_game()
