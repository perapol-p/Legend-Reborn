...
func save_summary() -> String:
	...
	if data.is_empty():
		return "Existing character  /  Level %d\nContinue from your home refuge." % progression().level if has_legacy_progress() else "No saved journey yet."
	var minutes: int = int(float(data.get("play_seconds", 0)) / 60.0)
	return "%s  /  LEVEL %d\n%dh %02dm played  /  Saved %s" % [area_name(str(data.scene)).to_upper(), data.character.level, int(minutes / 60.0), minutes % 60, data.get("saved_at", "")]
...
func continue_game() -> void:
	...
	if data.is_empty():
		...
		if data.scene == WORLD:
			...
			pending_loadout = data.duplicate(true)
		else:
			pending_restore = data
			if data.get("dungeon", {}) is Dictionary:
				dungeon_state = data.get("dungeon", {}).duplicate(true)
	start_world(HOME)

func start_world(scene_path: String = HOME) -> void:
	transitioning = true
	pause_menu.dismiss()
...
func register_player(new_player: Node) -> void:
	...
	if not pending_restore.is_empty():
		defeated.assign(pending_restore.get("defeated", []))
		restore_world(pending_restore)
		pending_restore.clear()
	else:
		# Ordinary death/rest resets the castle, while retaining the character build.
		defeated.clear()
	if not pending_loadout.is_empty():
		restore_loadout(pending_loadout)
		pending_loadout.clear()
	transitioning = false
	last_error = ""
...
func restore_loadout(data: Dictionary) -> void:
...
func enter_dungeon() -> bool:
	if transitioning or not is_instance_valid(player) or player.is_dead:
		return false
	if get_tree().current_scene.scene_file_path != HOME:
		return false
	var loadout := capture_world()
	if not save_game():
		return false
...
func return_home() -> bool:
	...
	dungeon_state.erase("dungeon")
	pending_loadout = snapshot.duplicate(true)
	pending_restore.clear()
	start_world(HOME)
	return true