...
@onready var current_health: float = float(total_health)
...
signal health_updated(current: float)
signal died
...
func _on_damage_signal(_by_what):
	...
	if health_bar_control:
		show_timer.start()
	var damage_power: float = float(_by_what.power)
	if hit_reporting_node and hit_reporting_node.has_method("get_node_or_null"):
		var stats_node = hit_reporting_node.get_node_or_null("PlayerStats")
		if stats_node:
			damage_power = stats_node.incoming_damage(damage_power)
	current_health = maxf(0.0, current_health - damage_power)
...
func _on_health_signal(_by_what):
	...
	if health_bar_control:
		show_timer.start()
	var healing_power: float = float(_by_what.power)
	if hit_reporting_node and hit_reporting_node.is_in_group("player"):
		healing_power *= 20.0
