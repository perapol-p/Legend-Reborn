...
var last_hit_by_player: bool = false
...
func hit(_by_who, _by_what):
	...
	if hurt_cool_down.is_stopped():
		...
		hurt_started.emit()
		last_hit_by_player = is_instance_valid(_by_who) and _by_who.is_in_group("player")
...
func death():
	...
	get_node("/root/GameSession").record_defeat(self)
	if last_hit_by_player:
		get_node("/root/CharacterProgression").add_experience(experience_reward)
